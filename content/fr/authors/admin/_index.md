---
title: Massyle DENDENE
name_pronunciation: ''
first_name: Massyle
last_name: DENDENE
status:
  icon: 🗿
superuser: true
highlight_name: true
role: Doctorant en mathématiques appliquées
organizations:
  - name: CREST
    url: https://crest.science/
profiles:
  - icon: at-symbol
    url: 'mailto:massyle.dendene@ensae.fr'
    label: "M'écrire"
  - icon: brands/github
    url: https://github.com/massyle-d
  - icon: brands/linkedin
    url: https://www.linkedin.com/in/massyle-dendene/
  - icon: academicons/orcid
    url: https://orcid.org/0009-0006-0118-3728
interests:
  - Contrôle stochastique
  - EDP
  - Solutions de viscosité
  - Processus de Hawkes
  - Apprentissage bayésien
  - Risque cyber
  - Finance
education:
  - area: Doctorat en mathématiques appliquées
    institution: ENSAE
    icon: ""
    date_start: 2025-11-17
    date_end: 2028-11-17
    summary: |
      Thèse sur le contrôle stochastique de l'intensité d'un processus de Hawkes avec apprentissage bayésien. Encadré par Roxana Dumitrescu, Nicolas Baradel et Caroline Hillairet.
  - area: MSc Actuariat
    institution: ENSAE
    icon: ""
    date_start: 2023-09-01
    date_end: 2025-10-31
    summary: |
      GPA : 4.0/4.0

      Spécialisation en actuariat.

  - area: MSc Finance quantitative
    institution: Université Paris Cité
    icon: ""
    date_start: 2024-09-01
    date_end: 2025-10-31
    summary: |
      Master de finance mathématique.

  - area: Diplôme d'ingénieur
    institution: IMT Atlantique
    icon: ""
    date_start: 2021-09-01
    date_end: 2023-08-31
    summary: |
      Parcours Ingénieur généraliste (majeure : ingénierie mathématique et computationnelle).

  - area: Licence Mathématiques & Économie
    institution: Université de Bretagne Occidentale (UBO)
    icon: ""
    date_start: 2022-09-01
    date_end: 2023-08-31
    summary: |
      Licence en mathématiques et économie.

work:
  - position: Stagiaire en recherche quantitative
    company_name: Exiom Partners
    company_url: 'https://www.exiompartners.com/'
    icon: ''
    date_start: 2025-05-19
    date_end: 2025-10-31
    summary: |
      - Sujet de recherche : tarification d'options européennes sur un marché illiquide — approche par diffusion à sauts subordonnée, avec des applications à l'assurance-vie et aux outils de gestion de bilan.

  - position: Stagiaire actuaire
    company_name: AON Luxembourg
    company_url: 'https://www.aon.com/luxembourg/default.jsp'
    icon: ''
    date_start: 2024-06-10
    date_end: 2024-09-29
    summary: |
      - Provisionnement pour des captives de réassurance non-vie.

  - position: Stagiaire de recherche
    company_name: Université Claude Bernard Lyon 1 (ISFA)
    company_url: 'https://isfa.univ-lyon1.fr/'
    icon: ''
    date_start: 2023-04-07
    date_end: 2023-08-25
    summary: |
      - Projet AXA JRI (Joint Research Initiative) : sélection de modèles de longévité et détection de ruptures pendant la période COVID-19 (détection de ruptures, sélection de modèles, risque de longévité).
      - Projet assurance tempêtes de grêle : cartographie des trajectoires de grêle en France en intégrant la taille des grêlons, modélisation des paramètres et génération de cartes sous Python.

skills:
  - name: Compétences techniques
    items:
      - name: Python, R & Git
        description: ''
        percent: 70
        icon: code-bracket
  - name: Centres d'intérêts
    color: '#eeac02'
    color_border: '#f0bf23'
    items:
      - name: Piano
        description: ''
        percent: 150
        icon: custom/piano
      - name: Tennis de table
        description: ''
        percent: 70
        icon: custom/tt
      - name: Don du sang
        description: ''
        percent: 46
        icon: custom/blood

languages:
  - name: Kabyle
    percent: 100
  - name: Français
    percent: 100
  - name: Anglais
    percent: 100
  - name: Allemand
    percent: 50
  - name: Italien
    percent: 2
---
Je suis doctorant au CREST (ENSAE, IP Paris) en mathématiques appliquées, avec un fort intérêt pour le contrôle stochastique, les probabilités et leurs applications à la finance et à l'assurance. Pendant mes études à l'ENSAE et au master M2MO, j'ai développé un intérêt particulier pour les processus stochastiques, la décision optimale sous incertitude et la gestion des risques fondée sur les modèles.

En dehors de la recherche, j'aime jouer du piano et pratiquer le ping-pong. Je prends également plaisir à lire et découvrir de nouvelles choses sur l'histoire des mathématiques.
